...
public class PlanCheckerMongoCollection<TDocument> implements MongoCollection<TDocument> {
  ...
  private void check(
    ...
    new PlanCheckerFindIterable<>(
            collection.find(filter).hint(hint).sort(sort).collation(collation), checker)
        .check();
  }
  ...
  private void check(
    ...
    new PlanCheckerFindIterable<>(
            collection.find(clientSession, filter).hint(hint).sort(sort).collation(collation),
            checker)
        .check();
  }
